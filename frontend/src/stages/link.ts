...
export async function link(
  ...
  // Ask the model how to connect. A single-source cold compile usually needs nothing.
  const decision = await gateway.extract({
    label: "link",
  ...
  // 3 · reconcile outlines — one Brief-shaped outline per goal over the merged graph.
  ...
  const outlines = ctx.goals.map((g) => ({
    ...
    fidelity: "claimed",
    scenes: [{ scene_id: `s-${g.id}`, title: g.text.slice(0, 80), node_ids: allNodeIds, ...(g.done_when ? { done_when: g.done_when } : {}), fidelity: "claimed" }],
  }));
